...
// counterDP counts the number of events (supports all event types)
type counterDP struct {
	attrs pcommon.Map

	count int64
}

func newCounterDP(attrs pcommon.Map) *counterDP {
	return &counterDP{
		attrs: attrs,
	}
}

func (dp *counterDP) Count(count int64) {
	dp.count += count
}

func (dp *counterDP) Copy(
	timestamp time.Time,
	dest pmetric.NumberDataPoint,
) {
	dp.attrs.CopyTo(dest.Attributes())
	dest.SetIntValue(dp.count)
