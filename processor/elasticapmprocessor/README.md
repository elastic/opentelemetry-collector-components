...
| `host_ip_enabled` | Controls whether the `host.ip` attribute should be set using the client address. When `true`, the processor will set the `host.ip` attribute from the client address when the mapping mode is "ecs".                                                                      | No       | `true`  |
