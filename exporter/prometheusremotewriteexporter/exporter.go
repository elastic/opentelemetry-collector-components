...
func (p *prwTelemetryOtel) setNumberConsumer(ctx context.Context, n int64) {
	p.telemetryBuilder.ExporterPrometheusremotewriteConsumers.Add(ctx, n, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwTelemetryOtel) recordRemoteWriteSentBatch(ctx context.Context) {
	p.telemetryBuilder.ExporterPrometheusremotewriteSentBatches.Add(ctx, 1, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwTelemetryOtel) recordTranslationFailure(ctx context.Context) {
	p.telemetryBuilder.ExporterPrometheusremotewriteFailedTranslations.Add(ctx, 1, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwTelemetryOtel) recordTranslatedTimeSeries(ctx context.Context, numTS int) {
	p.telemetryBuilder.ExporterPrometheusremotewriteTranslatedTimeSeries.Add(ctx, int64(numTS), metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwTelemetryOtel) recordWrittenSamples(ctx context.Context, numSamples int64) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWrittenSamples.Add(ctx, numSamples, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwTelemetryOtel) recordWrittenHistograms(ctx context.Context, numHistograms int64) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWrittenHistograms.Add(ctx, numHistograms, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwTelemetryOtel) recordWrittenExemplars(ctx context.Context, numExemplars int64) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWrittenExemplars.Add(ctx, numExemplars, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}
...
// The exporterhelper queue (configured via sending_queue) handles batching and per-tenant sharding;
// WAL and the internal goroutine pool have been removed in favour of this standard mechanism.
...
// Shutdown is a no-op: the exporterhelper queue handles in-flight requests.
...
func (prwe *prwExporter) handleRequests(ctx context.Context, input chan *prompb.WriteRequest) error {
	...
	for {
		select {
		case <-ctx.Done():
			return multierr.Append(errs, consumererror.NewPermanent(ctx.Err()))
