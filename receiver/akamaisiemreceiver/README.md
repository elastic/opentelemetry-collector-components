...
Polls the [Akamai SIEM API](https://techdocs.akamai.com/siem-integration/reference/get-configid) for security events and emits them as OTel logs. Supports two output formats controlled by `output_format`:
...
- [Getting Started](#getting-started)
  ...
  - [Scenario 4: File Export (Testing / Archival)](#scenario-4-file-export-testing--archival)
  - [Scenario 5: Console Debug (Development)](#scenario-5-console-debug-development)
  - [Scenario 6: High-Throughput Production](#scenario-6-high-throughput-production)
...
When only one instance is needed, the name can be omitted: `transform:`, `batch:`, `resource:`. The examples in this README use either plain names (`transform`, `batch`) or descriptive suffixes (`transform/raw`) to clarify the pipeline role.
...
- **`transform` processor** (required for raw mode Elasticsearch pipelines) — Uses [OTTL](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/transformprocessor) statements to set `elastic.mapping.mode: bodymap` as a scope attribute and copy `data_stream.*` fields into the body map. The processor is called `transform` (name it whatever you like). See [The `transform/raw` processor](#1-the-transformraw-processor-required-for-elasticsearch) for a detailed explanation of why each statement is needed. Not required for OTel mode — the exporter's default `otel` serialization handles everything automatically.
...
### Scenario 4: File Export (Testing / Archival)
...
### Scenario 5: Console Debug (Development)
...
### Scenario 6: High-Throughput Production
...
| `otelcol_akamai_siem_events_emitted` | counter | Total events emitted to pipeline |
...
| `otelcol_akamai_siem_mapping_errors` | counter | Total events that failed OTel mapping (otel mode only) |
...
| `otelcol_akamai_siem_page_processing_time` | histogram | Per-page processing time: NDJSON parse + mapping + ConsumeLogs (seconds) |
| `otelcol_akamai_siem_mapping_duration` | histogram | Time spent mapping events to output format (seconds) |
...
| Metric | Raw Mode | OTel Mode |
|---|---|---|
| **EPS** | ~15,600 | ~15,700 |
| CPU/event | 0.009ms | 0.037ms |
| RSS | 105 MB | 111 MB |
| Alloc/event | 1.3 KB | ~10 KB |
| CPU utilization | 12.8% | 50.8% |
| Goroutines | 12 | 12 |

EPS is flat across all modes — the receiver is I/O-bound (gzip decompression + NDJSON streaming).
