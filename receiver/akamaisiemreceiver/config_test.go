...
func validConfig() *Config {
	...
	cfg.HTTP.Endpoint = "https://akab-test.luna.akamaiapis.net"
	cfg.ConfigIDs = "12345"
...
func TestConfigDefaults(t *testing.T) {
	...
	assert.Equal(t, "logs", cfg.DataStream.Type)
	assert.Equal(t, "akamai.siem", cfg.DataStream.Dataset)
	assert.Equal(t, "default", cfg.DataStream.Namespace)
}

func TestConfigValidate_DataStreamRequired(t *testing.T) {
	cfg := validConfig()
	cfg.DataStream.Type = ""
	assert.ErrorContains(t, cfg.Validate(), "data_stream.type is required")

	cfg = validConfig()
	cfg.DataStream.Dataset = ""
	assert.ErrorContains(t, cfg.Validate(), "data_stream.dataset is required")

	cfg = validConfig()
	cfg.DataStream.Namespace = ""
	assert.ErrorContains(t, cfg.Validate(), "data_stream.namespace is required")
}
...
func TestConfigValidate_RejectsHTTPAuth(t *testing.T) {
	cfg := validConfig()
	authID := component.NewID(component.MustNewType("oauth2client"))
	cfg.HTTP.Auth = configoptional.Some(configauth.Config{AuthenticatorID: authID})
	assert.ErrorContains(t, cfg.Validate(), "http.auth is not supported")
}
