...
func TestReceiver_BodyIsRawJSON(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = fmt.Fprintln(w, `{"httpMessage":{"start":"1000","host":"test.com","status":"200"}}`)
	...
	lr := rl.ScopeLogs().At(0).LogRecords().At(0)
	assert.Equal(t, 0, lr.Attributes().Len(), "log record should have no attributes")

	// Body is a map with raw JSON in "message" key.
	assert.Contains(t, bodyMessage(t, lr), `"host":"test.com"`)
}

// TestReceiver_FullFlow tests the complete poll flow:
// Mock Akamai API → EdgeGrid auth → NDJSON parse → cursor persist
// → plog.Logs with body map {message: rawJSON, data_stream.*}.
func TestReceiver_FullFlow(t *testing.T) {
	...
	for i := 0; i < records.Len(); i++ {
		...
		assert.Equal(t, 0, lr.Attributes().Len(), "record %d should have no attributes", i)
	}
...
// TestEmitEvents_BodyShape verifies that emitEvents produces a body map with
// a "message" key containing the raw JSON string and data_stream.* body keys
// for Kibana filters.
func TestEmitEvents_BodyShape(t *testing.T) {
	cfg := createDefaultConfig().(*Config)
	sink := &consumertest.LogsSink{}
	...
	v, ok := body.Get("message")
	require.True(t, ok, "body map should have 'message' key")
	assert.Equal(t, `{"test":"event"}`, v.Str())

	for k, want := range map[string]string{
		"data_stream.type":      "logs",
		"data_stream.dataset":   "akamai.siem",
		"data_stream.namespace": "default",
	} {
		got, ok := body.Get(k)
		require.True(t, ok, "body map should have %q key", k)
		assert.Equal(t, want, got.Str(), "body[%q]", k)
	}
}

// TestEmitEvents_ScopeAndResource verifies that emitEvents sets the bodymap
// mapping mode on the scope and writes data_stream.* to resource attributes
// for ES exporter routing.
func TestEmitEvents_ScopeAndResource(t *testing.T) {
	cfg := createDefaultConfig().(*Config)
	cfg.DataStream.Dataset = "akamai.siem.tenant_a"
	sink := &consumertest.LogsSink{}
	rcv, err := newAkamaiReceiver(cfg, receivertest.NewNopSettings(NewFactory().Type()), sink)
	require.NoError(t, err)

	err = rcv.emitEvents(context.Background(), []string{`{"x":1}`})
	require.NoError(t, err)
	...
	for k, want := range map[string]string{
		"data_stream.type":      "logs",
		"data_stream.dataset":   "akamai.siem.tenant_a",
		"data_stream.namespace": "default",
	} {
		got, ok := rattr.Get(k)
		require.True(t, ok, "resource attr %q missing", k)
		assert.Equal(t, want, got.Str(), "resource attr %q", k)
	}

	scope := rl.ScopeLogs().At(0).Scope()
	mode, ok := scope.Attributes().Get("elastic.mapping.mode")
	require.True(t, ok, "scope attribute elastic.mapping.mode missing")
	assert.Equal(t, "bodymap", mode.Str())
}

// bodyMessage extracts the "message" string from a LogRecord body map.
