...
func TestReceiver_EmitFailure_NoCursorPersist(t *testing.T) {
	...
	require.NoError(t, err)
	rcv.setRawConsumer(failConsumer)
	require.NoError(t, rcv.Start(context.Background(), mockHost(memClient)))
...
func TestDualMode_BothConsumersReceiveEvents(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = fmt.Fprintln(w, `{"attackData":{"rule":"950004"},"httpMessage":{"host":"dual.test","start":"1700000000","status":"200","method":"GET","requestHeaders":"User-Agent: test","path":"/api","query":"q=1","port":"443","protocol":"HTTP/1.1","bytes":"512","responseHeaders":"Content-Type: application/json"},"geo":{"country":"US","city":"NYC","asn":"1234"},"userRiskData":{}}`)
		_, _ = fmt.Fprintln(w, `{"offset":"dual-cursor","total":1,"limit":10000}`)
	}))
	defer server.Close()
	...
	// Create the shared receiver via factory twice — same connection key, different output formats.
	ecsCfg := *cfg
	...
	ecsRcv, err := NewFactory().CreateLogs(context.Background(), set, &ecsCfg, ecsSink)
	require.NoError(t, err)

	otelRcv, err := NewFactory().CreateLogs(context.Background(), set, &otelCfg, otelSink)
	require.NoError(t, err)

	// Both should be the same shared component.
	...
	// Wait for both sinks to receive events.
	assert.Eventually(t, func() bool {
		return ecsSink.LogRecordCount() >= 1 && otelSink.LogRecordCount() >= 1
	}, 10*time.Second, 100*time.Millisecond, "both sinks should receive events")
	...
	require.NotEmpty(t, otelLogs)
	otelRecord := otelLogs[0].ResourceLogs().At(0).ScopeLogs().At(0).LogRecords().At(0)
	otelAttrs := otelRecord.Attributes()
	_, hasMethod := otelAttrs.Get("http.request.method")
	assert.True(t, hasMethod, "OTel should have http.request.method attribute")

}

// TestDualMode_SlowestWins_CursorNotAdvancedOnFailure verifies that if one
// consumer fails, the cursor does NOT advance (slowest-wins semantics).
func TestDualMode_SlowestWins_CursorNotAdvancedOnFailure(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = fmt.Fprintln(w, `{"httpMessage":{"host":"fail.test","start":"1700000000","status":"200","method":"GET"}}`)
		_, _ = fmt.Fprintln(w, `{"offset":"should-not-persist","total":1,"limit":10000}`)
	}))
	defer server.Close()
	...
	// Create receiver directly to test with a failing consumer.
	rcv, err := newAkamaiReceiver(cfg, receivertest.NewNopSettings(NewFactory().Type()))
	require.NoError(t, err)

	ecsSink := &consumertest.LogsSink{}
	rcv.setRawConsumer(ecsSink)

	failCfg := *cfg
	failCfg.OutputFormat = "otel"
	rcv.setOTelConsumer(&failingConsumer{})

	require.NoError(t, rcv.Start(context.Background(), mockHost(memClient)))

	// Give time for one poll to execute.
	time.Sleep(2 * time.Second)
	require.NoError(t, rcv.Shutdown(context.Background()))

	// Cursor should NOT have been persisted because OTel consumer failed.
	cursorData, err := memClient.Get(context.Background(), "akamai_siem_cursor")
	require.NoError(t, err)
...
func TestDualMode_SingleOTelBackwardCompat(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = fmt.Fprintln(w, `{"attackData":{"rule":"456"},"httpMessage":{"host":"otel.test","start":"1700000000","status":"200","method":"GET","path":"/test","protocol":"HTTP/1.1","bytes":"100","requestHeaders":"User-Agent: test","responseHeaders":"X-Test: 1","port":"443","query":""},"geo":{"country":"DE","city":"Berlin","asn":"5678"},"userRiskData":{}}`)
		_, _ = fmt.Fprintln(w, `{"offset":"otel-cursor","total":1,"limit":10000}`)
	}))
	defer server.Close()
	...
	lr := logs[0].ResourceLogs().At(0).ScopeLogs().At(0).LogRecords().At(0)
	_, hasMethod := lr.Attributes().Get("http.request.method")
	assert.True(t, hasMethod, "OTel mode should produce structured attributes")
}

// TestDualMode_SharedComponentSameKey verifies two instances with the same
// connection identity share a single receiver (LoadOrStore returns same component).
func TestDualMode_SharedComponentSameKey(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
	}))
	defer server.Close()
	...
	baseCfg.Endpoint = server.URL
	baseCfg.ConfigIDs = "shared-test"
	baseCfg.Authentication = EdgeGridAuth{
		ClientToken:  configopaque.String("shared-ct"),
		ClientSecret: configopaque.String("shared-cs"),
		AccessToken:  configopaque.String("shared-at"),
	}
	baseCfg.PollInterval = 24 * time.Hour
	...
	r1, err := factory.CreateLogs(context.Background(), set, &ecsCfg, &consumertest.LogsSink{})
	require.NoError(t, err)

	r2, err := factory.CreateLogs(context.Background(), set, &otelCfg, &consumertest.LogsSink{})
	require.NoError(t, err)

	// Both should start and shutdown without error (shared component handles ref counting).
...
// TestDualMode_DifferentKeysSeparateReceivers verifies that two instances with
// different endpoints create separate receivers (no sharing).
func TestDualMode_DifferentKeysSeparateReceivers(t *testing.T) {
	...
	cfg1.Endpoint = server1.URL
	cfg1.ConfigIDs = "1"
	cfg1.Authentication = EdgeGridAuth{
		ClientToken: configopaque.String("ct1"), ClientSecret: configopaque.String("cs1"), AccessToken: configopaque.String("at1"),
	}
	cfg1.PollInterval = 24 * time.Hour

	cfg2 := createDefaultConfig().(*Config)
	cfg2.Endpoint = server2.URL
	cfg2.ConfigIDs = "2"
	cfg2.Authentication = EdgeGridAuth{
		ClientToken: configopaque.String("ct2"), ClientSecret: configopaque.String("cs2"), AccessToken: configopaque.String("at2"),
	}
	cfg2.PollInterval = 24 * time.Hour

	set := receivertest.NewNopSettings(NewFactory().Type())
	factory := NewFactory()

	r1, err := factory.CreateLogs(context.Background(), set, cfg1, &consumertest.LogsSink{})
	require.NoError(t, err)
	r2, err := factory.CreateLogs(context.Background(), set, cfg2, &consumertest.LogsSink{})
	require.NoError(t, err)
...
// TestEmitEvents_NoConsumers verifies emitEvents returns an error when no consumers are set.
func TestEmitEvents_NoConsumers(t *testing.T) {
	cfg := createDefaultConfig().(*Config)
	rcv, err := newAkamaiReceiver(cfg, receivertest.NewNopSettings(NewFactory().Type()))
	require.NoError(t, err)
...
// TestEmitEvents_DualMode_BothSucceed verifies both consumers receive data.
func TestEmitEvents_DualMode_BothSucceed(t *testing.T) {
	cfg := createDefaultConfig().(*Config)
	rcv, err := newAkamaiReceiver(cfg, receivertest.NewNopSettings(NewFactory().Type()))
	require.NoError(t, err)
	...
	events := []string{
		`{"attackData":{"rule":"test"},"httpMessage":{"host":"dual.test","start":"1700000000","status":"200","method":"GET","path":"/","protocol":"HTTP/1.1","bytes":"0","requestHeaders":"","responseHeaders":"","port":"80","query":""},"geo":{"country":"US","city":"LA","asn":"1"},"userRiskData":{}}`,
	}
...
// TestEmitEvents_DualMode_OneFailsReturnsError verifies that if one consumer fails,
// emitEvents returns an error (preventing cursor advance).
func TestEmitEvents_DualMode_OneFailsReturnsError(t *testing.T) {
	cfg := createDefaultConfig().(*Config)
	rcv, err := newAkamaiReceiver(cfg, receivertest.NewNopSettings(NewFactory().Type()))
	require.NoError(t, err)
	...
	events := []string{`{"httpMessage":{"host":"fail.test","start":"1700000000","status":"200","method":"GET","path":"/","protocol":"HTTP/1.1","bytes":"0","requestHeaders":"","responseHeaders":"","port":"80","query":""},"geo":{},"userRiskData":{}}`}
