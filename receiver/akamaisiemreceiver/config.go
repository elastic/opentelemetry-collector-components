...
const (
	...
	defaultDataStreamDataset   = "akamai.siem"
	defaultDataStreamNamespace = "default"
)

// Config defines the configuration for the Akamai SIEM receiver.
type Config struct {
	// HTTP exposes the standard confighttp.ClientConfig fields (endpoint, tls,
	// timeout, headers, compression, …) at the receiver root via the squash tag.
	// Use `endpoint:` directly under the receiver to set the Akamai API host
	// (e.g. "https://akab-xxx.luna.akamaiapis.net"). The `auth` field is rejected
	// in Validate — credentials must be configured via the authentication block.
	HTTP confighttp.ClientConfig `mapstructure:",squash"`
	...
	// DataStream controls the data_stream.{type,dataset,namespace} fields the
	// receiver writes to resource attributes (for ES exporter routing) and to
	// the body map (for Kibana filters in bodymap mode).
...
// DataStreamConfig controls the Elasticsearch data stream this receiver targets.
// Defaults to logs / akamai.siem / default.
...
func createDefaultConfig() component.Config {
	return &Config{
		...
		DataStream: DataStreamConfig{
			Type:      defaultDataStreamType,
			Dataset:   defaultDataStreamDataset,
			Namespace: defaultDataStreamNamespace,
		},
...
func (c *Config) Validate() error {
	...
	if c.DataStream.Type == "" {
		return errors.New("data_stream.type is required")
	}
	...
	// Reject http.auth: the receiver wraps the configured transport with the
	// EdgeGrid signer, so any http.auth authenticator would run after EdgeGrid
	// and silently invalidate the request signature. Use the top-level
	// authentication block for EdgeGrid HMAC-SHA256 credentials instead.
	if c.HTTP.Auth.HasValue() {
		return errors.New("http.auth is not supported; this receiver authenticates via Akamai EdgeGrid HMAC-SHA256 (configure credentials under the authentication block)")
	}
	return nil
}
