...
// TODO: unify these code with esexporter
...
func newElasticsearchClient(
	...
	httpClient, err := config.Elasticsearch.ToClient(ctx, host, telemetry)
	if err != nil {
		return nil, err
	}
	...
	esLogger := clientLogger{
		...
		logRequestBody:  config.Elasticsearch.TelemetrySettings.LogRequestBody,
		logResponseBody: config.Elasticsearch.TelemetrySettings.LogResponseBody,
	}

	maxRetries := defaultMaxRetries
	if config.Elasticsearch.Retry.MaxRetries != 0 {
		maxRetries = config.Elasticsearch.Retry.MaxRetries
	}
	...
	return elasticsearch.NewClient(elasticsearch.Config{
		...
		// configure sniffing
		DiscoverNodesOnStart:  config.Elasticsearch.Discovery.OnStart,
		DiscoverNodesInterval: config.Elasticsearch.Discovery.Interval,
...
func createElasticsearchBackoffFunc(config *elasticsearchexporter.RetrySettings) func(int) time.Duration {
	if !config.Enabled {
		return nil
	}
