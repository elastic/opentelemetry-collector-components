...
```shell
docker run -it docker.elastic.co/observability-ci/otelbench:v0.4.0 -endpoint-otlp=http://172.17.0.1:4317 -api-key some_api_key -insecure
```
...
```shell
docker run -it --volume /path/to/config.yaml:/config.yaml docker.elastic.co/observability-ci/otelbench:v0.4.0 -endpoint-otlp=http://172.17.0.1:4317 -api-key some_api_key -insecure -config=/config.yaml
```
