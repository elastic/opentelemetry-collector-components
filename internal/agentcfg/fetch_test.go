...
type fetcherMock struct {
	fetchFn func(context.Context, Query) (Result, error)
}

func (f *fetcherMock) Fetch(ctx context.Context, query Query) (Result, error) {
...
func TestDirectConfigurationPrecedence(t *testing.T) {
	for _, tc := range []struct {
		...
		f := fetcherMock{
			fetchFn: func(_ctx context.Context, query Query) (Result, error) {
				return matchAgentConfig(query, tc.agentConfigs), nil
			},
		}
