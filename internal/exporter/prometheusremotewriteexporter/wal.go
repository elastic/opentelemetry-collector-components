...
func (p *prwWalTelemetryOTel) recordWALWriteLatency(ctx context.Context, durationMs int64) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalWriteLatency.Record(ctx, durationMs, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwWalTelemetryOTel) recordWALWrites(ctx context.Context) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalWrites.Add(ctx, 1, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwWalTelemetryOTel) recordWALWritesFailures(ctx context.Context) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalWritesFailures.Add(ctx, 1, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwWalTelemetryOTel) recordWALReadLatency(ctx context.Context, durationMs int64) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalReadLatency.Record(ctx, durationMs, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwWalTelemetryOTel) recordWALReads(ctx context.Context) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalReads.Add(ctx, 1, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwWalTelemetryOTel) recordWALReadsFailures(ctx context.Context) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalReadsFailures.Add(ctx, 1, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwWalTelemetryOTel) recordWALBytesWritten(ctx context.Context, bytes int) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalBytesWritten.Add(ctx, int64(bytes), metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwWalTelemetryOTel) recordWALBytesRead(ctx context.Context, bytes int) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalBytesRead.Add(ctx, int64(bytes), metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}

func (p *prwWalTelemetryOTel) recordWALLag(ctx context.Context, lag int64) {
	p.telemetryBuilder.ExporterPrometheusremotewriteWalLag.Record(ctx, lag, metric.WithAttributeSet(attribute.NewSet(p.otelAttrs...)))
}
