...
func createMetricsExporter(ctx context.Context, set exporter.Settings,
	...
	// When sending_queue is explicitly configured, pass it directly to exporterhelper
	// Otherwise fall back to the legacy remote_write_queue.
	var qCfg configoptional.Optional[exporterhelper.QueueBatchConfig]
	if prwCfg.SendingQueue.HasValue() {
		qCfg = prwCfg.SendingQueue
	} else {
		numConsumers := 1
